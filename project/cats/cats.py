...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def select(paragrahs):
        spilt_paragrahs = []
        spilt_paragrahs = split(lower(remove_punctuation(paragrahs)))
        for i in topic:
            utils_topic = lower(remove_punctuation(i))
            if utils_topic in spilt_paragrahs:
                return True
        return False
    return select
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    if lt == 0 and lr != 0: # check for special case
        return 0.0
    elif lt != 0 and lr == 0: # check for special case
        return 0.0
    elif lt == 0 and lr == 0: # check for special case
        return 100.0
    else:
        ...
        for j in reference_words:
            try:
                ...
                break
        return (count_for_accuracy/ lt) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        ...
        for i in valid_words:
            ...
            if diff < lowest_diff:
                lowest_diff = diff
                j = i # assign lowest diffrence word to j
...
def sphinx_switches(start, goal, limit):
    ...
    if limit < 0:
        return 0
    # make sure start and goal has same length
    if len(start) < len(goal):
        add_str = ' ' * (len(goal) - len(start))
        start = ''.join([start, add_str])
    elif len(start) > len(goal):
        add_str = ' ' * (len(start) - len(goal))
        goal = ''.join([goal, add_str])
    # recursion
...
def report_progress(typed, prompt, user_id, send):
    ...
    send_dic['id'] = user_id
    send_dic['progress'] = progress_ratio
    send(send_dic)
...
def time_per_word(times_per_player, words):
    ...
    while i < len(times_per_player):
        ...
        while j < len(times_per_player[i]) - 1:
            times_tmp.append(times_per_player[i][j + 1] - times_per_player[i][j])
            j += 1
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    time_tmp = all_times(game)
    shortest_time = []
    fastest_w = []
    # create a list which contains the shortest time for each word
    for i in word_indices:
        shortest = 100
        for j in player_indices:
            if time_tmp[j][i] < shortest:
                shortest = time_tmp[j][i]
        shortest_time.append(shortest)
    # main
    for i in player_indices:
        fastest_w_tmp = []
        word_num = -1
        for j in word_indices:
            if time(game, i, j) == shortest_time[j]:
                fastest_w_tmp.append(word_at(game, j))
                # check if (word_at(game, j)) already in another player's list
                word_num += 1
                k = 0
                while k < i:
                    if word_at(game, j) in fastest_w[k]:
                        del fastest_w_tmp[word_num]
                        word_num -= 1
                    k += 1
        fastest_w.append(fastest_w_tmp)
    return fastest_w
    # END PROBLEM 10
