...
class Ant(Insect):
    ...
    food_cost = 0
    is_buffed = False  # attributes for EC
    # ADD CLASS ATTRIBUTES HERE
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if self.can_contain(place.ant) == True:
                ...
                place.ant = self
            elif place.ant.can_contain(self) == True:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while current_place != beehive:
            ...
            if current_place.bees != [] and self.min_range <= count_range <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        damage_amount = 0
        # check if fireAnt's health dropped to 0
        if amount < self.health:
            damage_amount = amount
        else:
            damage_amount = amount + self.damage
        # create a shadow list
        list_bee = self.place.bees[:]
        for bee in list_bee:
...
# BEGIN Problem 6
class WallAnt(Ant):
    """WallAnt has a large amount of health that can be used as wall."""
...
# BEGIN Problem 7
class HungryAnt(Ant):
    """HungryAnt eat a random bee from its place but take time to chew."""
    ...
    def action(self, gamestate):
        """check if HungryAnt is chewing. if not, eat a random bee in its place"""
...
class TankAnt(ContainerAnt):
    """TankAnt project the ant in its place and deal damage to all bees in its place"""
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is a kind of ThrowerAnt that can be placed on water"""
...
# END Problem EC
    ...
    def __init__(self, health = 1):
        # BEGIN Problem EC
        super().__init__(health)
        global imposer_queen_indicator
        imposer_queen_indicator += 1
        if imposer_queen_indicator > 1:
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN Problem EC
        if self.is_imposer == True:
            self.reduce_health(self.health)
        else:
            super().action(gamestate)
            behind_current_place = self.place.exit
            # try buff both container ant and contained ant
            # buff() is a method in Class Ant
            while behind_current_place != None:
                if behind_current_place.ant != None:
                    behind_current_place.ant.buff()
                    if behind_current_place.ant.is_container() == True and \
                        behind_current_place.ant.contained_ant != None:
                        behind_current_place.ant.contained_ant.buff()
                behind_current_place = behind_current_place.exit
        # END Problem EC
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem EC
        super().reduce_health(amount)
        if self.is_imposer == False:
            bees_win()
        # END Problem EC

    def remove_from(self, place):
        """real queen cannont be removed"""
        if self.is_imposer == True:
