...
def more_boar(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4 
    i = check_digits(player_score)
    j = check_digits(opponent_score)
    a1 = 0
    a2 = 0
    b1 = 0
    b2 = 0
    if i == 1: #check singular digit
        a1 = take_digits(player_score, (i + 1))
        a2 = take_digits(player_score, i)
    else:
        a1 = take_digits(player_score, i)
        a2 = take_digits(player_score, (i - 1))
    if j == 1: #check singular digit
        b1 = take_digits(opponent_score, (j + 1))
        b2 = take_digits(opponent_score, j)
    else:
        b1 = take_digits(opponent_score, j)
        b2 = take_digits(opponent_score, (j -1))
    if a1 < b1 and a2 < b2:
        return True
    else:
        return False
...
def take_digits(score, n):
    """Return a certain digit from score"""
    i = 0
    d = 0
    while i < n:
        d = score % 10
...
def check_digits(score):
    """check how many digits in score"""
    if score >= 100:
        return 3
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        ...
        if more_boar(score0, score1) == True:
            who = 0
        elif more_boar(score1, score0) == True:
            who = 1
        else:    
            who = next_player(who)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    def say(score0, score1):
        high = running_high # create a new variable to hold running_high
        a = last_score # create a new variable to hold last_score
        if who == 0:
            b = score0 - last_score
            a = score0
            if b > high:
                high = b
                print('Player 0 has reached a new maximum point gain.', high, 'point(s)!')
        else:
            b = score1 - last_score
            a = score1
            if b > high:
                high = b
                print('Player 1 has reached a new maximum point gain.', high, 'point(s)!')
        return announce_highest(who, a, high)
    return say
    # END PROBLEM 7
